...
r2 = 0.5
nSamples = 1000
theta = np.linspace(0, 30*np.pi, nSamples, endpoint = False)
phi = np.sqrt(3)*theta
...
nClus = 6
theta = np.linspace(0, 2*np.pi, int(nClus/2), endpoint = False)
clus0 = np.vstack((r1*np.cos(theta), r1*np.sin(theta), r2 + 0.0*theta)).transpose()
clus1 = np.vstack((r1*np.cos(theta), r1*np.sin(theta), -r2 + 0.0*theta)).transpose()
clus0 = np.concatenate((clus0, clus1), axis = 0)
kmeans = KMeans(n_clusters = nClus, random_state = 0, init = clus0).fit(X)
...
for i in range(nClus):
    ...
    enStruct = [32, 64, 32, 16, 4, nDim]
    enAct = ['elu', 'elu', 'elu', 'elu', 'elu', None]
    deStruct = [4, 16, 32, 64, 32, X.shape[1]]
    deAct = ['elu', 'elu', 'elu', 'elu', 'elu', None]
    ...
    trainArgs = {'epochs': 2000, 'batch_size': X[clusPts[i], :].shape[0]}
...
theta = np.linspace(0, 30*np.pi*nsteps/nSamples, nsteps, endpoint = False)
phi = np.sqrt(3)*theta
X = np.vstack(((r1 + r2*np.cos(phi))*np.cos(theta), (r1 + r2*np.cos(phi))*np.sin(theta), r2*np.sin(phi))).transpose()
...
# Extract theta and phi from trajectory, compare to what they should be
thetaTraj = np.arctan2(xArr[:, 1], xArr[:, 0])
phiTraj = np.arcsin(xArr[:, 2]/r2)
fig, ax = plt.subplots()
for i in np.unique(clusArr):
    ax.scatter(x[np.squeeze(clusArr == i)], thetaTraj[np.squeeze(clusArr == i)],
                facecolor = plt.cm.Set1(i),
                s = 20, linewidth = 1, zorder = 2)
    ax.scatter(x[np.squeeze(clusArr == i)], phiTraj[np.squeeze(clusArr == i)],
                facecolor = plt.cm.Set1(i), marker = 'v',
                s = 20, linewidth = 1, zorder = 2)
ax.plot(np.mod(theta + np.pi, 2*np.pi) - np.pi, color = (0.8, 0.8, 0.8), zorder = 1)
ax.plot(np.arcsin(np.sin(phi)), color = (0.8, 0.8, 0.8), zorder = 1)

ax.set_xlabel('time step')
ax.set_ylabel('$\\theta, \\phi$')
